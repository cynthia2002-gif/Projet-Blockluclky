...
contract BlockLuckyLoterie {
    ...
    uint public frequenceTirage; // Fréquence de tirage en jours
    ...
    constructor(uint _minParticipants, uint _frequenceTirageEnJours, address _charityAddress) {
        manager = msg.sender;
        ticketPrice = 2 * 10**15; // 2 finney
        prixticket = ticketPrice / 10**15;
        minParticipants = _minParticipants;
        frequenceTirage = _frequenceTirageEnJours; // Durée de chaque période de tirage
        deadline = block.timestamp + (frequenceTirage * 1 days); // Initialisation de la deadline
    ...
    function tirageAuSort() internal {
        ...
        // Réinitialise pour le prochain tirage
        delete participants;
        deadline = block.timestamp + (frequenceTirage * 1 days); // Fixe la nouvelle date de fin pour le prochain tirage
    }
    ...
    function tirerGagnant() external onlyManager {
        require(block.timestamp >= deadline, "Il n'est pas encore l'heure du tirage");
        tirageAuSort();
    }
