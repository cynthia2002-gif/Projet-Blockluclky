...
const contractAddress = "0x8c1f808bF2Bb829Da07f6eC74a16384E6b7A5Ef7"; // Remplacez par l'adresse déployée
const contractABI = [
    {
        "inputs": [],
        "name": "acheterTicket",
...
async function afficherCagnotte() {
  ...
  async function afficherGagnants() {
    try {
      const gagnants = await contract.methods.getGagnants().call(); // Récupère la liste des gagnants
      const gagnantsDropdown = document.getElementById("gagnants");
  
      // Effacer les anciennes options (sauf le premier élément)
      gagnantsDropdown.innerHTML = `<option value="">Sélectionnez un gagnant</option>`;
  
      // Ajouter chaque gagnant au menu déroulant
      gagnants.forEach((gagnant, index) => {
        const option = document.createElement("option");
        option.value = gagnant;
        option.textContent = `Gagnant ${index + 1} : ${gagnant}`;
        gagnantsDropdown.appendChild(option);
      });
    } catch (error) {
      console.error("Erreur lors de l'affichage des gagnants :", error);
    }
  }
...
setInterval(afficherGagnants, 1000);
